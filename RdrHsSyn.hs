...
mkPatSynMatchGroup (L loc patsyn_name) (L _ decls) =
  ...
  where
    fromDecl (L loc decl@(ValD (PatBind pat@(L _ (ConPatIn ln@(L _ name) details)) rhs))) =
...
{- -- TODO: SHAYAN
   ...
   NegApp (L l (HsOverLit pos_lit))
                        -> return (mkNPat (L l pos_lit))
-}
   ...
   ExplicitList es    -> do ps <- mapM (checkLPat msg) es
                            return (ListPat ps)
   ExplicitPArr es    -> do ps <- mapM (checkLPat msg) es
                            return (PArrPat ps)
...
splitBang (L _ (OpApp l_arg bang@(L _ (HsVar (L _ op))) r_arg))
        ...
        = do { mb_l <- go l es ann
             ; case mb_l of
                 Just (op', Infix, j : k : es', ann')
                   ...
                   where
                     op_app = L loc (OpApp k (L loc' (HsVar (L loc' op))) r)
                 _ -> return Nothing }
...
checkMonadComp :: P (HsStmtContext id)
checkMonadComp = do
    pState <- getPState
...
checkCmd _ (HsArrApp e1 e2 ptt b) =
    return $ HsCmdArrApp e1 e2 ptt b
...
checkCmd _ (HsIf ep et ee) = do
    pt <- checkCommand et
    pe <- checkCommand ee
    return $ HsCmdIf ep pt pe
checkCmd _ (HsLet lb e) =
    checkCommand e >>= (\c -> return $ HsCmdLet lb c)
checkCmd _ (HsDo DoExpr (L l stmts)) =
    mapM checkCmdLStmt stmts >>= (\ss -> return $ HsCmdDo (L l ss))

checkCmd _ (OpApp eLeft op eRight) = do
...
checkCmdStmt _ (LastStmt e s) =
    checkCommand e >>= (\c -> return $ LastStmt c s)
checkCmdStmt _ (BindStmt pat e) =
    checkCommand e >>= (\c -> return $ BindStmt pat c)
checkCmdStmt _ (BodyStmt e) =
    checkCommand e >>= (\c -> return $ BodyStmt c)
...
mk_rec_upd_field (HsRecField (L loc (FieldOcc rdr)) arg)
  = HsRecField (L loc (Unambiguous rdr)) arg
